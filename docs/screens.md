...
Idle screens are defined in `config/screens.alb`, an ALP bundle that maps IDs to screen plugins. The firmware loads this bundle on idle and cycles through the screens in order of their ATTR sections.
...
| CONFIG (0x03) | `"id"` | Nested ALP bundle with config | no       |
...
The optional CONFIG section contains a nested ALP bundle with stored config values (see `docs/config.md` "Stored Values" format) that are passed as arguments to the screen plugin.
