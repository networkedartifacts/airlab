...
stm_entry_t* stm_query(bool exclaim, stm_action_t action) {
  ...
  for (size_t i = 0; i < stm_num; i++) {
    ...
    // check co2
    if (entry->co2_min != 0 && (!sample.ok || sample.co2 < entry->co2_min)) {
      entry->selected = false;
    } else if (entry->co2_max != 0 && (!sample.ok || sample.co2 > entry->co2_max)) {
      entry->selected = false;
    }

    // check temperature
    if (entry->tmp_min != 0 && (!sample.ok || sample.tmp < entry->tmp_min)) {
      entry->selected = false;
    } else if (entry->tmp_max != 0 && (!sample.ok || sample.tmp > entry->tmp_max)) {
      entry->selected = false;
    }

    // check humidity
    if (entry->hum_min != 0 && (!sample.ok || sample.hum < entry->hum_min)) {
      entry->selected = false;
    } else if (entry->hum_max != 0 && (!sample.ok || sample.hum > entry->hum_max)) {
      entry->selected = false;
    }
